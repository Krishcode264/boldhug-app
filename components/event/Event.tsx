    ...
    <View className="w-full py-4  bg-gray-100 shadow-slate-800 shadow-xl p-2 mb-4 px-4 rounded-md flex gap-2">
      ...
      <View className="flex-row items-center gap-4 px-2 py-1.5 rounded-2xl bg-gray-200">
        <Image
          ...
          source={{uri: event.host.profilePhoto}}
        />
        <Text className="text-lg font-semibold">{event.host.name}</Text>
      </View>
 </Button>
      <Text className="font-bold text-lg px-2  w-[95%]">
        {event.description}
      </Text>
      <View className="flex-row items-center p-1   gap-2 ">
        <MapPin color={"black"} width={25} height={25}  />
        <Text className="">{event.location.name}</Text>
      </View>
      <View className="flex-row gap-3  flex-wrap">
        <EventInfoTag info={`${event.filledSlots} / ${event.totalSlots}`} />
        <EventInfoTag info={event.time} />
        <EventInfoTag info={event.date} />
        <Button
