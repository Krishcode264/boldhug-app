...
const EditprofilePhoto = () => {
  ...
  const { user: editUser, setState } = useEditUserState();
  ...
  const handleSelectPhoto = async () => {
    ...
    if (file.assets?.length && file.assets.length > 0) {
      setState({ profilePhoto: file.assets[0], name: "" });
    }
  };
  ...
  const handleSubmitProfileData = async () => {
    ...
    try {
      setLoading({ isLoading: true, message: "updating profile" });
      const res = await api.patch("/user", { data: { name: editUser?.name } });

      const photoUpload = await api.get("/user/profile-photo/upload-url", {
        params: {
          fileName: editUser?.profilePhoto?.fileName,
          type: editUser?.profilePhoto?.type,
        },
      });
      ...
      if (photoUpload.data.url) {
        ...
        const fileData = await FileSystem.readAsStringAsync(
          editUser?.profilePhoto?.uri as string,
          {
        ...
        const res = await axios.put(photoUpload.data.url, fileBuffer, {
          headers: { "Content-Type": editUser?.profilePhoto?.type },
        });
      ...
      setErr({
        ...
        error:
          err instanceof AxiosError
            ...
            : "something went wrong updating profile data , try again",
      });
    }
  };
    ...
    return (
      <View className="border-violet-300  rounded-full p-2">
        <Image
          ...
          style={{}}
          source={{
            uri: editUser?.profilePhoto?.uri || user?.profilePhoto?.url,
          }}
        />
        <Button
          style={{ top: 40 }}
  ...
  return (
    <SafeAreaWrapper className="p-4 ">
      ...
      <View className="p-2  h-[70%] items-center justify-center gap-8">
        <View
          className="bg-violet-200  rounded-full"
          style={{ width: width / 1.8, height: width / 1.8 }}
        >
          {editUser?.profilePhoto || user?.profilePhoto ? (
            <ProfilePhoto />
          ) : (
            <>
              <Button
        ...
        <Button
          ...
          disabled={editUser?.profilePhoto ? false : true || loading.isLoading}
      ...
      {loading.message && (
        ...
        <Text className="text-violet-500  mx-auto ">{loading.message}</Text>
      )}
      {err.isError && (
        <ErrorModal
          error={err.error}
          shouldRetry={true}
          retry={handleSubmitProfileData}
          setVisible={setErr}
        />
      )}
    </SafeAreaWrapper>
